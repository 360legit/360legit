...
import { blogPosts, resourceCategories } from '../data/siteData'
          ...
          <Reveal delay={200}>
            <div className="flex flex-col lg:flex-row items-center justify-between gap-8 mb-16 p-8 bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/40">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button className="px-5 py-2 rounded-full bg-orange-600 text-white text-xs font-bold transition-all">All Posts</button>
                {resourceCategories.map((category) => (
                  <button key={category} className="px-5 py-2 rounded-full bg-slate-50 text-slate-500 text-xs font-bold hover:bg-orange-50 hover:text-orange-600 transition-all border border-transparent hover:border-orange-100">
                    {category}
                  </button>
                ))}
              </div>
            ...
            <article className="group relative rounded-[3rem] overflow-hidden bg-[#1B3942] mb-20 shadow-2xl">
              <div className="grid lg:grid-cols-2">
                <div className="aspect-[16/9] lg:aspect-auto overflow-hidden">
                  <img 
                    src="https://images.unsplash.com/photo-1454165833767-027eeea160d7?q=80&w=2070&auto=format&fit=crop" 
                    alt="Featured analysis" 
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                  />
                </div>
                <div className="p-10 md:p-16 flex flex-col justify-center text-white">
                  <span className="text-orange-400 font-bold uppercase tracking-[0.2em] text-[10px] mb-6 block">Special Report</span>
                  <h2 className="font-serif text-3xl md:text-5xl font-bold mb-6 leading-tight group-hover:text-orange-300 transition-colors">
                    The 2024 Compliance<br />Roadmap for Saas Startups
                  </h2>
                  <p className="text-slate-300 text-lg mb-10 leading-relaxed max-w-lg">
                    A deep dive into cross-border taxation, GST reconciliation, and automation-first filing systems for the modern Saas era.
                  </p>
                  <Link to="/contact" className="inline-flex items-center gap-3 font-bold text-white hover:text-orange-400 hover:gap-4 transition-all uppercase tracking-widest text-xs">
                    Read The Full Report <FiArrowRight size={18} />
                  </Link>
                </div>
              </div>
            </article>
